<template>
  <div class="mapbox">
    ...
    <div id="buttons">
      <GoCart style="width:50%; height: 100%;"></GoCart>
      <b-button v-b-toggle.sidebar-right style="width:50%; height: 100%;">Wish List</b-button>
    </div>
    <b-sidebar id="sidebar-right" title="Sidebar" width="40%" right shadow backdrop>
      <div class="px-3 py-2">
        {{ $store.state.list.cart_name }}
        <ShoppingBox :list="this.$store.state.list" style="height: 100%;"></ShoppingBox>
        <b-button @click="gotolist">리스트 수정하기</b-button>
      </div>
    </b-sidebar>
  </div>
</template>

<template>
  <b-modal id="modal-prevent-closing" ref="login_modal" title="Sign in" @show="resetModal" @hidden="resetModal"
    @ok="handleOk" size="xl">
    <p style="text-align: center;">
      <img src="@/assets/favicon.png" alt="">
    </p>
    <form ref="ID" @submit.stop.prevent="handleSubmit">
      <b-form-group label="Name" label-for="inputNum" invalid-feedback="Name is required" :state="nameState">
    ...
    <form ref="PW" @submit.stop.prevent="handleSubmit">
      <b-form-group label="Password" label-for="inputPass" invalid-feedback="password is required" :state="passState">
        <b-form-input id="inputPass" type="password" value="" @focus="onInputFocus" @input="onInputChange"
          v-model="password" :state="passState" required></b-form-input>
      </b-form-group>
    </form>
    <!-- {{ name }}
    {{ password }} -->
    <div class="keybaord-area">
      <SimpleKeyboard @onChange="onChange" @onKeyPress="onKeyPress" :input="inputName" :inputName="inputName">
      </SimpleKeyboard>
    </div>
  </b-modal>
</template>
